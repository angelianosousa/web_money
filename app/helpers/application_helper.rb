...
module ApplicationHelper
  ...
  def filter
    link_to '#', class: 'btn btn-outline-dark btn-sm btn-round my-3', style: 'font-size:12px;', type: 'button',
                 data: {
                   toggle: 'collapse',
                   target: '#collapseSearch'
                 },
                 aria: {
                   expanded: false,
                   controls: 'collapseSearch'
                 } do
  ...
  def link_to_delete_resource(text, link)
    link_to link, class: 'btn-sm', style: 'border-radius: 10px;', method: :delete,
