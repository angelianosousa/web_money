...
RSpec.describe 'Bills', type: :request do
  ...
  describe 'POST /bills' do
    context 'Success Scenario' do
      let(:bill_attributes) { attributes_for(:bill, user: user) }
    ...
    context 'Fail Scenarios' do
      let(:bill_attributes_invalid) do
        attributes_for(:bill, user: user, title: nil, date: nil, price_cents: -1)
      end
  ...
  describe 'PUT | PATCH /bills/:id' do
    context 'Success Scenario' do
      let(:new_bill_attributes) { attributes_for(:bill, user: user, title: 'My title') }
