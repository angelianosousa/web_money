  ...
  describe '#call' do
    ...
    context 'Success scenario' do
      ...
      describe 'create a transaction expense' do
        let(:transaction_params) do
          attributes_for(:transaction, user_profile_id: user_profile.id, account_id: account.id, move_type: :expense)
        end
        let(:transaction) { CreateTransaction.call(user_profile, transaction_params) }
    ...
    context 'Fail scenario' do
      let(:transaction_params) do
        attributes_for(:transaction, account_id: account.id, price_cents: nil, date: nil, user_profile_id: nil)
      end
      let(:transaction) { CreateTransaction.call(user_profile, transaction_params) }
      ...
      describe 'when given invalid transaction params' do
        it 'should not be valid' do
          ...
          expect(transaction.errors.messages[:account]).to      include 'não é válido'
